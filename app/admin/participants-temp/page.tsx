...
import { Users, Search, Check, X, Eye, Mail, Phone, MapPin, Calendar, FileText, Download } from 'lucide-react'
...
export default function TempParticipantsManagement() {
  ...
  const fetchParticipants = async () => {
    try {
      const response = await fetch('/api/admin/participants-temp')
      if (response.ok) {
  ...
  const exportToExcel = async () => {
    try {
      await ExcelExporter.exportToExcel({
        filename: 'المشاركين_المؤقت.xlsx',
        sheetName: 'المشاركين',
        columns: [
          ...
          { key: 'teamRole', header: 'الدور المفضل', width: 15 },
          { key: 'experience', header: 'الخبرة', width: 20 },
          { key: 'skills', header: 'المهارات', width: 30 },
        ...
        data: filteredParticipants.map(participant => ({
          ...
          skills: Array.isArray(participant.skills) ? participant.skills.join(', ') : participant.skills,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#c3e956]/10 to-[#3ab666]/10 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            <h1 className="text-4xl font-bold text-[#01645e] mb-2">المشاركين المسجلين</h1>
            <p className="text-[#8b7632] text-lg">عرض وإدارة المشاركين المسجلين في النظام (تخزين مؤقت)</p>
          </div>
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                جميع المشاركين المسجلين في النظام
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>الدور المفضل</TableHead>
                ...
                <TableBody>
                  {filteredParticipants.map((participant) => (
                    <TableRow key={participant.id}>
                      ...
                      <TableCell>
                        <div className="font-medium">{participant.teamRole}</div>
                      </TableCell>
                      <TableCell>
                              ...
                              {selectedParticipant && (
                                <div className="space-y-4">
                                  <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        ...
                                        <div>
                                          <strong>الدور المفضل:</strong> {selectedParticipant.teamRole}
                                        </div>
                                        <div className="flex items-center">
                                  ...
                                  {selectedParticipant.additionalInfo && (
                                    <div>
                                      <h4 className="font-semibold text-[#01645e] mb-2">معلومات إضافية</h4>
                                      <div className="space-y-2 text-sm">
                                        {selectedParticipant.additionalInfo.experience && (
                                          <div>
                                            <strong>الخبرة:</strong> {selectedParticipant.additionalInfo.experience}
                                          </div>
                                        )}
                                        {selectedParticipant.additionalInfo.skills && (
                                          <div>
                                            <strong>المهارات:</strong> {selectedParticipant.additionalInfo.skills}
                                          </div>
                                        )}
                                        {selectedParticipant.additionalInfo.motivation && (
                                          <div>
                                            <strong>الدافعية:</strong> {selectedParticipant.additionalInfo.motivation}
                                          </div>
                                        )}
                                      </div>
                                    </div>

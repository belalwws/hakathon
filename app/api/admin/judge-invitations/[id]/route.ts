...
// DELETE /api/admin/judge-invitations/[id] - Cancel invitation
export async function DELETE(
  ...
  try {
    ...
    const invitation = await prisma.judgeInvitation.update({
      where: { id: params.id },
...
// PATCH /api/admin/judge-invitations/[id] - Resend invitation
export async function PATCH(
  ...
  try {
    ...
    if (action === 'resend') {
      ...
      const currentInvitation = await prisma.judgeInvitation.findUnique({
        where: { id: params.id }
      })
      ...
      const invitationLink = `${baseUrl}/judge/register?token=${invitation.token}`
